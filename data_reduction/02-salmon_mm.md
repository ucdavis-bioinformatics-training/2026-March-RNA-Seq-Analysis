...
## Salmon quantification using alignment input

**Salmon can be used to perform quantification on bam files generated by alignment programs, such as STAR. In this case, salmon requires the alignment to be in transcriptome space. In STAR, one can add "TranscriptomeSAM" to the "--quantMode" parameter to have STAR translate the alignment from genomic coordinates to transcriptomic coordinates. Then the output transcriptome bam files can be used by salmon for quantification.**
